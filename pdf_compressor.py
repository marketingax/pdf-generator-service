...
This module provides PDF compression functionality optimized for cloud deployment environments.
It uses PyMuPDF (fitz) instead of Ghostscript to avoid external binary dependencies that
can be problematic in containerized deployments like Railway.

The compression approach focuses on:
1. Image optimization - identifying and compressing embedded images
2. Structural optimization - removing unused objects and compressing streams
3. Smart scaling - reducing oversized images while maintaining quality
4. Fallback handling - graceful degradation if compression doesn't help

Key Features:
- No external binary dependencies (Ghostscript-free)
- Configurable compression levels (0-4)
- Intelligent image processing with size thresholds
- Comprehensive error handling and logging
- File size reporting and comparison

The compression process:
1. Opens PDF with PyMuPDF for direct manipulation
2. Extracts and analyzes embedded images
3. Applies JPEG compression and scaling to suitable images
4. Replaces original images with compressed versions
5. Applies general PDF optimizations (garbage collection, deflation)
6. Compares file sizes and keeps the better result

Edge Cases:
- Very small images are skipped to avoid quality degradation
- Images that don't compress well are left unchanged
- If overall compression doesn't reduce file size, original is kept
- Missing dependencies are handled gracefully

This approach is particularly effective for PDFs with large embedded images,
which is common in template and design documents.
"""

"""
 * === pdf_compressor.py ===
 * Updated: September 18, 2025 - 12:00 PM
 * Summary: Cloud-friendly PDF compression using PyMuPDF for file size reduction
 * Key Components:
 *   - compress_pdf_file(): Main compression function with configurable levels
 *   - compress_image(): PIL-based image optimization
 *   - get_file_size(): File size utilities and formatting
 * Dependencies:
 *   - Requires: PyMuPDF (fitz), PIL for image processing
 * Version History:
 *   v1.0 - Initial PyMuPDF-based implementation
 * Notes:
 *   - No Ghostscript dependency for cloud compatibility
 *   - Intelligent image processing with quality preservation
 *   - Comprehensive error handling and logging
 */